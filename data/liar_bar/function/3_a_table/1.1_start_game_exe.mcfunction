...
#先设置模式为模式1
$execute if score gambling_mode jijifujiji_liar_bar_variable matches 1 run scoreboard players enable @a[x=$(x1),y=$(y1),z=$(z1),dx=18,dy=6,dz=18] jijifujiji_liar_bar_a_table_click
#让范围内的玩家可以点按钮
...
#创建队伍
execute unless score gambling_mode jijifujiji_liar_bar_variable matches 1 run scoreboard players set @p jijifujiji_liar_bar_a_table_click 101
