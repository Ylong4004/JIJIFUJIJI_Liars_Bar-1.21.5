...
tag @a[team=liar_bar_a_table,tag=!a_table_last] add a_table__shoot
#如果出了恶魔牌
tag @a[team=liar_bar_a_table] remove a_table_playing
#清除正在玩的标签
execute at @a[team=liar_bar_a_table,tag=a_table__shoot,limit=1] run playsound minecraft:entity.tnt.primed master @a ~ ~ ~ 1 0.97
#播放点燃炸药的音效
