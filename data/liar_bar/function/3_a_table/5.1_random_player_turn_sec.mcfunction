...
execute if score liar_bar_a_table_time jijifujiji_liar_bar_variable matches 75 if entity @a[team=liar_bar_a_table,tag=a_table_playing] as @a[team=liar_bar_a_table,tag=a_table_playing] run return run function liar_bar:3_a_table/6.3_select_deal_time_over with storage minecraft:jijifujiji_liat_bar a_table_position
#如果当前玩家存在，时间到了，自动给你随机出牌
execute if score liar_bar_a_table_time jijifujiji_liar_bar_variable matches 75 unless entity @a[team=liar_bar_a_table,tag=a_table_playing] run function liar_bar:3_a_table/96_offline_reset with storage minecraft:jijifujiji_liat_bar a_table_position
