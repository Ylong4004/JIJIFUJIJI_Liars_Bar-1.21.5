...
execute store result score liar_bar_a_table_receive jijifujiji_liar_bar_variable run clear @s minecraft:prize_pottery_sherd[minecraft:item_model="minecraft:jijifujiji_fantuan"] 1
...
#没钱，没加入成功，提示重新加入
execute if score liar_bar_a_table_receive jijifujiji_liar_bar_variable matches 0 run tellraw @s [{text:"点击加入",color:"yellow"},{text:"【赌饭团币】",color:"gold",bold:true,click_event:{action:"run_command",command:"/trigger jijifujiji_liar_bar_a_table_join set 100"},hover_event:{action:"show_text",value:"点我加入赌饭团币模式对局"}},{text:"骗子酒馆",color:"yellow"}]
#重新提示加入信息
