...
bossbar set jijifujiji_liar_bar_e_table_time name [{text:"请选择模式（任意玩家）    ",color:"yellow"},{"score":{"objective":"jijifujiji_liar_bar_variable","name":"liar_bar_e_table_temp"},color:"gold",bold:true},{text:"s",color:"gold"}]
$bossbar set jijifujiji_liar_bar_e_table_time players @a[x=$(x1),y=$(y1),z=$(z1),dx=18,dy=6,dz=18]
#更新boss条初始状态

execute if score liar_bar_e_table_time jijifujiji_liar_bar_variable matches 20 run function liar_bar:3_e_table/99_over_game with storage minecraft:jijifujiji_liat_bar e_table_position
#如果到了时间就结束
