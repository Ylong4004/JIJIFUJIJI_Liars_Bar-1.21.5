...
#没钱，没加入成功，提示重新加入
execute if score liar_bar_e_table_receive jijifujiji_liar_bar_variable matches 0 run tellraw @s [{text:"点击加入",color:"yellow"},{text:"【赌饭团币】",color:"gold",bold:true,click_event:{action:"run_command",command:"/trigger jijifujiji_liar_bar_e_table_join set 100"},hover_event:{action:"show_text",value:"点我加入赌饭团币模式对局"}},{text:"骗子酒馆",color:"yellow"}]
#重新提示加入信息
...
$execute if score liar_bar_e_table_receive jijifujiji_liar_bar_variable matches 1 run tellraw @a[x=$(x1),y=$(y1),z=$(z1),dx=18,dy=6,dz=18] [{selector:"@s",color:"dark_green",bold:true},{text:"支付了一个饭团币，加入对局！",color:"green",bold:true}]
#如果扣除成功
