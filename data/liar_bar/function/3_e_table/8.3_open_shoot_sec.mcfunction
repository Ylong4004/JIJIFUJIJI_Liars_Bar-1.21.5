...
#6阶段每秒一次
scoreboard players set liar_bar_e_table_temp jijifujiji_liar_bar_variable 110
#临时变量=110
...
#execute if score liar_bar_e_table_time jijifujiji_liar_bar_variable matches 110 run say 1
execute if score liar_bar_e_table_time jijifujiji_liar_bar_variable matches 110 run function liar_bar:3_e_table/4.0_deal_cards.exe with storage minecraft:jijifujiji_liat_bar e_table_position
#时间到了 运行重新发牌
...
#标签集 
# tag=e_table_playing  正在玩的a桌玩家
# tag=e_table_1、tag=e_table_2 、tag=e_table_3 、tag=e_table_4    a桌1/2/3/4玩家
# tag=e_table_card_select  玩家选择出的牌
# tag=e_table_card_on_center 玩家出了得牌
# tag=e_table_last  a桌上一个玩家
# tag=e_table__shoot 应该被枪毙的人
#队伍集
# team=liar_bar_e_table  a桌队伍
