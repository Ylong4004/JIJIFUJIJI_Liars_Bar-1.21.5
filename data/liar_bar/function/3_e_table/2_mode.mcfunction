...
execute if score @s jijifujiji_liar_bar_e_table_click matches 101 run data modify storage minecraft:jijifujiji_liat_bar e_table_position.mode set value "面包"
...
#重置掉按了模式按钮玩家的按键计分板
scoreboard objectives add jijifujiji_liar_bar_e_table_join trigger ["A桌的加入检测"]
#新建一个按键检测用于加入游戏
$scoreboard players enable @a[x=$(x1),y=$(y1),z=$(z1),dx=18,dy=6,dz=18] jijifujiji_liar_bar_e_table_join
#让范围玩家可以选择加入
...
#设置为初始tick0
scoreboard players set liar_bar_e_table_time jijifujiji_liar_bar_variable 20
#秒为20
scoreboard players set liar_bar_e_table_number jijifujiji_liar_bar_variable 0
#设置已加入玩家数量为0
