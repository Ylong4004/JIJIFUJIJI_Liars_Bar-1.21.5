...
#重置掉按了模式按钮玩家的按键计分板
tag @e[tag=b_table_card_on_center,tag=b_table_card_select] remove b_table_card_on_center
tag @e remove b_table_card_select
#清除前面加的标签
...
execute if score liar_bar_b_table_stage jijifujiji_liar_bar_variable matches 4 as @a[team=liar_bar_b_table,tag=b_table_playing] run function liar_bar:3_b_table/5.2_random_player_turn_show with storage minecraft:jijifujiji_liat_bar b_table_position
execute if score liar_bar_b_table_stage jijifujiji_liar_bar_variable matches 5 as @a[team=liar_bar_b_table,tag=b_table_playing] run function liar_bar:3_b_table/7.2_standard_turn_show with storage minecraft:jijifujiji_liat_bar b_table_position
